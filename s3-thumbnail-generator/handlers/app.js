...
const upload = multer({
    ...
    encoding: 'binary' // Specify encoding used to encode the input data
});
...
exports.uploadImageAndProcessThumbnail = async (event, context) => {
    const { body } = event;
    const { image } = await upload.single('imageBinary')(event['body-json']);
    ...
    const response = {
        statusCode: 200,
        message: `Event : ${event}, Image bin: ${image.buffer}, Bucket Name: ${bucket}`,
    };

    return response;

    // console.log(`Image Key: ${body.imageKey} Image Path: ${body.file.path}`);
}